...
def generate_launch_description():
    package_name = 'mybot_moveit_config'

    robot_name_in_model = 'mybot'


    urdf_tutorial_path = get_package_share_directory('mybot_description')
    pkg_share = FindPackageShare(package=package_name).find(package_name) 
    urdf_model_path = os.path.join(pkg_share, f'config/kr210_gazebo.urdf.xacro')
    default_world_path = urdf_tutorial_path + '/world/my_world.world'
    ...
    # 设置GAZEBO_MODEL_PATH,临时添加world文件夹作为模型搜索路径
    ...
    gazebo =  ExecuteProcess(
        cmd=['gazebo', '--verbose', default_world_path, '-s', 'libgazebo_ros_init.so', '-s', 'libgazebo_ros_factory.so'],
        output='screen')
